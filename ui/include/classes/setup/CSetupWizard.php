...
class CSetupWizard extends CForm
{
	...
	private function doAction(): void
	{
		...
		if ($this->getStep() == self::STAGE_REQUIREMENTS) {
			...
			$input = [
				...
				'DB_ENCRYPTION' => (bool) getRequest(
					'tls_encryption',
					$this->getConfig('DB_ENCRYPTION', $config->config['DB']['ENCRYPTION'])
				),
				'DB_ENCRYPTION_ADVANCED' => (bool) getRequest(
					'verify_certificate',
			...
			switch ($this->getConfig('DB_CREDS_STORAGE')) {
				case DB_STORE_CREDS_VAULT_HASHICORP:
					$this->setConfig('DB_VAULT_URL', getRequest(
						'vault_url',
						$this->getConfig('DB_VAULT_URL', CVaultHashiCorp::API_ENDPOINT_DEFAULT)
					));

					$this->setConfig('DB_VAULT_PREFIX', getRequest(
						'vault_prefix_hashicorp',
						$this->getConfig('DB_VAULT_PREFIX', '')
					));

					$this->setConfig('DB_VAULT_DB_PATH', getRequest(
						'vault_db_path',
				...
				case DB_STORE_CREDS_VAULT_CYBERARK:
					...
					$vault_certificates = (bool) getRequest(
						'vault_certificates',
			...
			if (hasRequest('next') && array_key_exists(self::STAGE_DB_CONNECTION, getRequest('next'))) {
				switch ($this->getConfig('DB_CREDS_STORAGE')) {
					case DB_STORE_CREDS_VAULT_HASHICORP:
						$vault_provider = new CVaultHashiCorp(
					...
					case DB_STORE_CREDS_VAULT_CYBERARK:
						$vault_provider = new CVaultCyberArk(
							$this->getConfig('DB_VAULT_URL'),
							$this->getConfig('DB_VAULT_PREFIX'),
							$this->getConfig('DB_VAULT_DB_PATH'),
	...
	private function stageDbConnection(): array
	{
		...
		$table = (new CFormList())
			...
			->addRow(_('Database port'), [
				(new CNumericBox(
					'port',
					$this->getConfig('DB_PORT', $config->config['DB']['PORT']),
					5,
					false,
					false,
					false
		...
		$table
			...
			->addRow(
				...
				(new CTextBox(
					'vault_prefix_cyberark',
					$db_creds_storage == DB_STORE_CREDS_VAULT_CYBERARK
						? $this->getConfig('DB_VAULT_PREFIX')
						: ''
				))
					->setWidth(ZBX_TEXTAREA_MEDIUM_WIDTH)
		...
		if ($this->step_failed) {
			$message_box = makeMessageBox(
				ZBX_STYLE_MSG_BAD,
				CMessageHelper::getMessages(),
				_('Cannot connect to the database.'),
				false,
				true
			);
		} else {
	...
	private function stageSummary(): array
	{
		...
		if ($server_name !== '') {
			$table
				->addRow(null)
				->addRow(
					(new CSpan(_('Advantal server name')))->addClass(ZBX_STYLE_GREY),
	...
	private function stageInstall(): array
	{
		...
		if ($this->getConfig('DB_CREDS_STORAGE', DB_STORE_CREDS_CONFIG) == DB_STORE_CREDS_VAULT_HASHICORP) {
			...
			$vault_provider = new CVaultHashiCorp(
				$vault_config['VAULT_URL'],
				$vault_config['VAULT_PREFIX'],
				$vault_config['VAULT_DB_PATH'],
			...
			$vault_provider = new CVaultCyberArk(
				...
				$vault_config['VAULT_PREFIX'],
				$vault_config['VAULT_DB_PATH'],
				$vault_config['VAULT_CERT_FILE'],
				$vault_config['VAULT_KEY_FILE']
			);
		...
		if ($error) {
			...
			$message_box = makeMessageBox(
				ZBX_STYLE_MSG_BAD,
				$messages,
				_('Cannot create the configuration file.'),
				false,
				true
			);
			$message = [
