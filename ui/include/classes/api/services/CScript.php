...
class CScript extends CApiService
{
	...
	public function get(array $options)
	{
		...
		if ($db_scripts) {
			...
			$db_scripts = $this->unsetExtraFields(
				$db_scripts,
				['scriptid', 'groupid', 'host_access'],
				$options['output']
			);
		...
		foreach ($scripts as &$script) {
			...
			if ($script['scope'] == ZBX_SCRIPT_SCOPE_HOST || $script['scope'] == ZBX_SCRIPT_SCOPE_EVENT) {
				...
				if ($script['manualinput'] == ZBX_SCRIPT_MANUALINPUT_ENABLED) {
					$script += array_intersect_key($db_script, array_flip([
						'manualinput_validator_type',
						'manualinput_validator'
					]));
			...
			if ($script['type'] != $db_script['type']) {
				switch ($script['type']) {
					...
					case ZBX_SCRIPT_TYPE_CUSTOM_SCRIPT:
						$script += array_intersect_key($defaults, array_flip([
							'authtype',
							'username',
							'publickey',
							'privatekey',
				...
				$script['type'] == ZBX_SCRIPT_TYPE_SSH && $script['authtype'] != $db_script['authtype']
		...
		foreach ($scripts as $index => $script) {
			if (($script['scope'] == ZBX_SCRIPT_SCOPE_HOST || $script['scope'] == ZBX_SCRIPT_SCOPE_EVENT)
				...
				if ($script['manualinput_validator_type'] == ZBX_SCRIPT_MANUALINPUT_TYPE_STRING) {
					...
					if (!preg_match(
						'/' . CRegexHelper::handleSlashEscaping($script['manualinput_validator']) . '/',
						...
						self::exception(
							...
							_s(
								...
								'/' . ($index + 1) . '/manualinput_default_value',
	 ...
	 * @param array $scripts
	 *
	 * @throws APIException if at least one script has execute_on parameter equal to Advantal server and global script
	 *                      execution is disabled by Advantal server.
	 */
	private static function checkScriptExecutionEnabled(array $scripts): void
	{
		if (!CSettingsHelper::isGlobalScriptsEnabled()) {
			foreach ($scripts as $index => $script) {
				if (
					$script['type'] == ZBX_SCRIPT_TYPE_CUSTOM_SCRIPT
					...
					self::exception(
						...
						_s(
							'Invalid parameter "%1$s": %2$s.',
							'/' . ($index + 1) . '/execute_on',
							_('global script execution on Advantal server is disabled by server configuration')
						)
					);
				}
			}
		...
		if ($db_action = DBfetch($db_actions)) {
			self::exception(
				...
				_s(
					...
					$db_scripts[$db_action['scriptid']]['name'],
					$db_action['name']
				)
			);
		}
	...
	public function execute(array $data)
	{
		...
		if (
			!CSettingsHelper::isGlobalScriptsEnabled() && $db_script['type'] == ZBX_SCRIPT_TYPE_CUSTOM_SCRIPT
		...
		$zabbix_server = new CZabbixServer(
			...
			timeUnitToSeconds(CSettingsHelper::get(CSettingsHelper::SCRIPT_TIMEOUT)),
	...
	public function getScriptsByEvents(array $options): array
	{
		...
		$macros_data = CMacrosResolverHelper::resolveManualEventActionScripts(
			$macros_data,
			$events,
			$manualinput_values
		);
	...
	protected function applyQueryOutputOptions($tableName, $tableAlias, array $options, array $sqlParts)
	{
		...
		if (
			$options['selectGroups'] !== null || $options['selectHostGroups'] !== null
			...
			foreach ($actions as $action) {
				foreach (['operations', 'recovery_operations', 'update_operations'] as $operation_type) {
					foreach ($action[$operation_type] as $operation) {
						if (
							array_key_exists('opcommand', $operation)
							...
							self::exception(
								...
								_s(
									...
									$action_scripts[$operation['opcommand']['scriptid']]['name'],
									$action['name']
								)
							);
						}
