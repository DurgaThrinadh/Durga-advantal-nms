...
jQuery(function ($) {
  ...
  var ServerChecker = {
    ...
    /**
     * Sends ajax request to get Advantal server availability and message to show if server is not available.
     */
    check: function () {
      new RPC.Call({
    ...
    showMessage: function (e) {
      if (
        !this.warning ||
        (e && (e.pageY < this.elem_offset_top || e.type === "mouseleave"))
      ) {
        $(document).off("mousemove.ServerChecker mouseleave.ServerChecker");

        this.warning = true;
        this.$elem.css("display", "flex").hide().fadeIn(200);
      }
    },
    ...
    hideMessage: function (e) {
      if (this.warning) {
        if (e && e.type === "mouseenter") {
          $(document).on(
            "mousemove.ServerChecker mouseleave.ServerChecker",
