...
class testAuditlogAction extends testAuditlogCommon
{
	...
	public function testAuditlogAction_Create()
	{
		...
		$operationid = CDBHelper::getAll(
			'SELECT operationid FROM operations WHERE actionid=' .
		...
		$op_group = CDBHelper::getRow(
			'SELECT opmessage_grpid FROM opmessage_grp WHERE operationid=' .
	...
	public function testAuditlogAction_Update()
	{
		...
		$operationid = CDBHelper::getRow(
			'SELECT operationid FROM operations WHERE (actionid, operationtype)=('
		...
		$updated = json_encode([
			...
			'action.name' => ['update', 'Updated action audit', 'Report problems to Advantal Administrators'],
